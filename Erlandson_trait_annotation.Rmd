...
erlandson_annotated <- read_excel("salix_rhizomorph.xlsx") %>% if_else(Species != NA, Species, "sp.") %>% unite(Ecto_ID, Genus:Species, sep = " ", remove = FALSE)

erlandson_metadata <- left_join(salix_raw, erlandson_annotated) %>% mutate (Tree_site = Plspp, Plspp = NULL)
...
short_traits <- read_xlsx("~/Fungi Textbooks/functional-fungi/FungalTraits 1.2_ver_16Dec_2020.xlsx") %>% mutate(Genus = GENUS) 
