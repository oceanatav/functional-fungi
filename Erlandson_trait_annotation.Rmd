...
erlandson_expl <- left_join(annotated_merge, exp_type) %>%  drop_na(Ecto_spp) %>% distinct(.) %>% mutate(
...
```{r}
trait_matrix <- erlandson_expl %>% select(Plspp, distance, Rhizomorph) %>% mutate(No.Tips = rep(1)) %>% group_by(Plspp,distance,Rhizomorph) %>%  summarize(count=n()) %>% mutate(short = rep(NA), medium = rep(NA), long=rep(NA), rhizo = rep(NA), norhizo = rep(NA), unknown = rep(NA))%>% ungroup() 
...
#need to repeat step to get distinct trees 
```
