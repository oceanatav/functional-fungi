...
```{r}
erlandson_OTU_raw <- readRDS("erl_OTU.rds")
erlandson_traits_raw <- readRDS("erl_trait_table.rds")
...
#erlandson_metadata_raw <- readRDS("erlandson_generated_metadata.rds") 
...
#  }
#}
#erlandson_metadata[523, 12] = "Thelephora terrestris"
#erlandson_metadata[585, 12] = "Thelephora terrestris"
#erlandson_metadata[664, 12] = "Thelephora terrestris"
#erlandson_metadata[671, 12] = "Tomentellopsis sp."
#saveRDS(erlandson_metadata, "erlandson_generated_metadata.rds")
erlandson_metadata <- readRDS("erlandson_generated_metadata.rds")
...
mendo_tejon_join <- bind_rows(mendo_names, tejon_names) %>% unite(Species, OUT_MatchwMendo, SpeciesCode) %>% unite(ID, species, BlastID) %>% rename(OTU = Species)
mendo_tejon_join <- as.data.frame(mendo_tejon_join)
mendo_tejon_join$OTU <- gsub("NA_", "", as.character(mendo_tejon_join$OTU))
mendo_tejon_join$OTU <- gsub("_NA", "", as.character(mendo_tejon_join$OTU))
...
mendo_tejon_join <- mendo_tejon_join[mendo_tejon_join$OTU != "NA",]

ref_ID <- mendo_tejon_join %>% distinct()
saveRDS(ref_ID, "master_ID_df.rds")
```
...
#saveRDS(erlandson_names, "erl_test.rds")
...
match_names <- function(OTU, ID, ref_ID){
  ...
  ref_ID <- bind_rows(ref_ID, input_df) %>% distinct(.) %>% select(-ref_name)
 # print(unique(input_df$ref_name))
  ref_ID <<- ref_ID
}
match_names(erlandson_names$Ecto_spp, erlandson_names$Ecto_ID, ref_ID)
saveRDS(ref_ID, "master_ID_df.rds")
dim(ref_ID)
for(i in 1:nrow(erlandson_names)) {
  for(j in 1:nrow(ref_ID)) {
    if(erlandson_names$Ecto_ID[i] == ref_ID$ID[j]) {
      erlandson_names$ref_name[i] = ref_ID$OTU[j] 
...
#No longer works bc ref_ID was updated.

#DONT FORGET TO WRITE SOMETHING THAT ADDS THE OTU COL AND ID TO REF_ID

#erlandson_OTU <- erlandson_OTU_raw
erlandson_OTU_combined <- erlandson_OTU %>% select(order(colnames(.))) %>%rename("17" = Cortinarius1,
         "3" = Cenococcum_geophilum) %>% rownames_to_column(var = "Tree_site")
...
label_erlandson <- erlandson_metadata %>% 
 group_by(Plot, 
           mean, 
           Tree_site, 
           Ecto_spp) %>% 
...
erlandson_OTU_add_enviro <- label_erlandson %>% 
  ...
  mutate(site = rep("cedar_creek", nrow(erlandson_OTU_combined))) %>% 
...
#erlandson_traits <- erlandson_traits_raw %>% rownames_to_column(var = "Tree_site") %>% transmute(Tree_site = Tree_site,short=  short, medium = medium,long = rep(0),rhizo= r_true,norhizo = r_false)


erlandson_rel_traits <- erlandson_traits_raw %>% 
  rownames_to_column(var = "Tree_site") %>% 
  ...
  transmute(Tree = Tree_site,
         ...
         rhizo = rhizo/sumAbund) %>% replace(is.na(.), 0)%>% # unknown = unknown/sumAbund,
  column_to_rownames(., var = "Tree")
erlandson_rel_traits

#ERLANDSON OTU

erlandson_rel_OTU <- erlandson_OTU_add_enviro %>%
  mutate(sumAbund = select(., Cadophora_finlandica:Tuber4) %>% 
             rowSums(na.rm = TRUE)) %>% 
  relocate(sumAbund, .after = enviro)

for(i in 5:ncol(erlandson_rel_OTU)) {
 erlandson_rel_OTU[,i] <- erlandson_rel_OTU[,i]/erlandson_rel_OTU$sumAbund
  }
erlandson_rel_OTU <- erlandson_rel_OTU %>%
  select(-sumAbund,-site,-enviro ) %>% column_to_rownames(var="Tree_site")
...
erl_tax <-erlandson_metadata %>% 
  ...
  column_to_rownames(., var = "Ecto_spp")
    ##OTU table
erl_otu <- otu_table(erlandson_OTU_matrix, taxa_are_rows = F) %>% replace(is.na(.), 0)
...
#MENDO
  ##Sample table
#Tree_sp_mendo <- pygmy.clean %>% select(Tree, Trait.HostTree) %>%  distinct()
...
#for(i in 1:nrow(mendo_tax)){
#  if(mendo_tax$Genus[i] == "Uncultured"){
 #   mendo_tax$Genus[i] = mendo_tax$Species[i]
  #  mendo_tax$Species[i] = "sp."}}
  # %>% distinct(., SpeciesCode) #%>% 
#mendo_tax <- mendo_tax %>%  column_to_rownames(., var = "SpeciesCode")
#saveRDS(mendo_tax, "mendo_tax.rds")
mendo_tax <- readRDS("mendo_tax.rds") %>%  transmute(Phylum = Phylum,
         Order = Order,
         Class = Class,
         Family = Family,
         Genus = Genus,
...
com_otu <- otu_table(combo_OTU_matrix, taxa_are_rows = F) 
  ##"Taxa" table
com_tax <- bind_rows(tej_tax, mendo_tax, erl_tax) %>% mutate(Kingdom = rep("Fungi")) %>% rownames_to_column(., var="OTU")
    ### Remove weird ellipses rownames : e.g. "25...144"
#1- figure out what the correct number is for species code (why do only some print with weird ellipses--potentially duplicates?) --LOOKS LIKE it is the first section that is valid. so remove everything starting with"."
#2- use gsub to remove either before or after &including "..."
#https://stackoverflow.com/questions/39897155/replace-and-remove-part-of-string-in-rownames 
#gsub("^[^-]+-|_[^_]+$", "", rownames(df))
com_tax$OTU <- sub("[...].*", "", com_tax$OTU)
com_tax <- com_tax %>% distinct(., OTU, .keep_all = T) %>% column_to_rownames(., var="OTU")
com_tax <- as.matrix(com_tax)
com_tax <- tax_table(com_tax)

  ##Sample table
com_sam_df <- bind_rows(mendo_sam, tej_sam,  erl_sam) %>% replace(is.na(.), 0)
com_sam <- sample_data(com_sam_df)

com_ps <- phyloseq(com_otu,com_tax,com_sam)
```
#Phyloseq objects for trait NMDS
```{r}
com_trait <- otu_table(combo_trait_matrix, taxa_are_rows = F)
com_trait_tax <- com_sam_df %>% rownames_to_column(., var = "Tree")

com_trait_temp <- as.data.frame(combo_trait_matrix ) 
com_trait_temp <- com_trait_temp %>% rownames_to_column(., var = "Tree")
com_trait_sam <- right_join(com_trait_tax, com_trait_temp) %>% column_to_rownames(., var = "Tree")
com_trait_sam <- sample_data(com_trait_sam)
...
dat <- c("short", "medium", "long", "norhizo", "rhizo")
rnames <- c("short", "medium", "long", "norhizo", "rhizo")
 cnames <- "Species"

com_trait_tax <- matrix(dat, nrow =5, ncol =1, dimnames=list(rnames,cnames))
com_trait_tax <- tax_table(com_trait_tax)
com_trait_ps <- phyloseq(com_trait_sam, com_trait, com_trait_tax)
```
...
p_com = plot_ordination(com_ps, com_ps_ord, type="species", color = "Order", shape = "site")
print(p_com)
...
p2_com_tr = plot_ordination(com_trait_ps, com_trait_ps_ord, type="samples", color="long", shape = "enviro") 
print(p2_com_tr)
#p2_com_tr + geom_polygon(aes(fill=site)) + geom_point(size=5) + ggtitle("site")
#TO PLOT BY TRAIT
#going to have to go back and take out line of code that sums tips abundances
#will have to make new unique rownames for samples, retaining (or creating) site column as well as enviro and unique trait.
p3_com_tr = plot_ordination(com_trait_ps, com_trait_ps_ord, type="taxa", color="Species") 
...
```
```{r}
p4_trait_ord <- plot_ordination(com_trait_ps, com_trait_ps_ord, type="taxa")  #Look into a shiny that lets u hover over points to see what is there! would be great to know what traits cluster together!!!
```
...
```
### Appendix##
```{r Surplus Code, eval=FALSE, include=FALSE}
