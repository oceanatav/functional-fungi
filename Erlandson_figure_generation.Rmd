...
mendo_rel_OTU <- mendo_OTU_table %>%
    mutate(sumAbund = select(., c(1:82,)) %>% 
             rowSums(na.rm = TRUE)) #%>% 
  #relocate(sumAbund, .after = enviro)

for(i in 1:ncol(mendo_rel_OTU)) {
...
combo_OTU_matrix <- matrix(rep(0,length(tree_list)*length(OTU_list)),nrow=length(tree_list),ncol=length(OTU_list))

colnames(combo_OTU_matrix) <- OTU_list
rownames(combo_OTU_matrix) <- tree_list
...
for(i in 1:dim(combo_OTU_matrix)[2]){
	...
	colnum <- 0
	for(j in 1:dim(tejon_OTU_matrix)[2]){
		if(colnames(tejon_OTU_matrix)[j]==OTU){
			colnum <- j 	}	}	
	if(colnum==0){ 	tejon.abun <- rep(0,dim(tejon_OTU_matrix)[1]) 	} 
	...
	for(j in 1:dim(mendo_OTU_matrix)[2]){
		if(colnames(mendo_OTU_matrix)[j]==OTU){
			colnum <- j  } 	}	
	if(colnum==0){	mendo.abun <- rep(0,dim(mendo_OTU_matrix)[1]) 	} 
