...
#Read in Trait Tables

```{r}
erlandson_traits <- read.csv("trait_table_erlandson.csv")

# Create new columns in df for traits
erlandson_traits$short <- 0
erlandson_traits$long <- 0
erlandson_traits$medium <- 0
erlandson_traits$norhizo <- 0
erlandson_traits$rhizo <- 0
...
##END FUNCTION
```
```{r Summary Table w/ Two Treatments}
######################################
...
```{r}
tejon.trees <- read_csv('TraitAnalysisforOceana/fungal-abundance.csv')
tejon.ecto <- read.csv('TraitAnalysisforOceana/ecto_OTUs.csv')
tejon.ecto.traits <- read.csv('TraitAnalysisforOceana/ecto_OTU_traits.csv')
dim(tejon.trees)
dim(tejon.ecto)
...
tejon.trees_sum<- rowSums(tejon.trees_sum[,3:2424])
...
tejon.tree.traits <- tejon.trees.ecto[,1:3] # subset out columns site, tree, enviro
...
# Consolidate Matrices for Mendocino + Tejon
