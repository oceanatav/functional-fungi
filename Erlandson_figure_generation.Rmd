...
# t/d save as and reload as R obj to no need to remove X column
```
...
# str_detect from tidyverse 
# merge names

# ALSO to write function, lets add Species column to dataset. 
# try to automate, get maximum number of spp. names on there

# see slack messages for code
# loop thru matrix, if row of selected matrix is match to reference (mendo?0), print OTU_ID that it mathces, if no, print original otu_ID
# 

#My for loop approach would be:
#x <- c(2,5,3,9,8,11,6)
#count <- 0
#for (val in x) {
#if(val %% 2 == 0)  {
...
#So, for your data it would be something like:
#for (row in 1:nrow(taxon_matrix)) {
#for (anotherrow in 1:nrow(another_taxon_matrix)) {
#if (taxon_matrix$taxon[row] == another_taxon_matrix$taxon[anotherrow]
#}

#taxon_matrix$ismatch[row] = TRUE
#}
...
pygmy.core<-aggregate(pygmy.clean$No.Tips,
                      list(pygmy.clean$Sample.ID,pygmy.clean$SpeciesCode,
                           pygmy.clean$Tree,
                           pygmy.clean$Core,
                           pygmy.clean$Terrace,
                           pygmy.clean$Pygmy,
