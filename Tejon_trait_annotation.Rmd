...
t2 <- tejon_OTU %>% select(-site, -enviro) %>% pivot_longer(., cols=c(2:118)) %>% rename(OTU_ID=name) %>% left_join(., tej_trait_total)

t3<- t2 %>% select(-OTU_ID, -trait) %>% unique() %>% group_by(tree, type) %>% summarize(., count=sum(value)) %>% mutate(
  type=if_else(type=="r_true", "rhizo", type),
  type=if_else(type=="r_false", "norhizo", type)) %>% ungroup()

#this is not quite getting me the result i want--I need to intrerfeer before dropping OTU to build the trait-function complexes But yay! finally free from that mysterious function
